...
class Pickup:
    ...
    def __calculate_price(self, parked_time):
        ...
        exceed_time = 0
        if max_stay_hours == 'Up to midnight' and self.__picking_up_ticket.exist_time.date() != self.__picking_up_ticket.arrive_time.date():
        ...
        elif parked_time > max_stay_hours:
            exceed_time = parked_time - max_stay_hours
    ...
    def __get_price_on_day(self):
        ...
        if len(rs) == 0:
            raise ParkingException.ParkingException("Some thing wrong with the price list.")        
        return np.float32(rs[0].price_per_hour), np.intc(rs[0].max_stay_hours)