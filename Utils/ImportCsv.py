...
from Data.ParkDay import ParkDay

# TODO: continue implementation
class ImportParkingPriceList:

    time_range_pattern = "\d{2}:\d{2} - (\d{2}:\d{2}|Midnight)$"
    ...
    def handle_time_span(self, arr):
        day = self.maapping_dow[arr[0]]
        for i in range(self.time_ranges):
            self.ParkDays.append(ParkDay(day, arr[self.time_ranges[i].max_stay_index], arr[self.time_ranges[i].price_per_hour_index]))
